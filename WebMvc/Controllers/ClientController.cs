...
namespace WebMvc.Controllers
{
    ...
    public class ClientController : Controller
    {
        ...
        {
            List<Todoitems> todos = null;
            HttpResponseMessage response = await client.GetAsync(WebApiPath);
            if (response.IsSuccessStatusCode)
            {
                todos = await response.Content.ReadAsAsync<List<Todoitems>>();  //requires System.Net.Http.Formatting.Extension
            }
            return View(todos);
        }
        ...
        public async Task<ActionResult> Details(int id)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                Todoitems todo = await response.Content.ReadAsAsync<Todoitems>();
                return View(todo);
            }
            return NotFound();
        }
        ...
        public async Task<ActionResult> Create([Bind("Id,Name,IsComplete")] Todoitems todo)
        {
        ...
        public async Task<ActionResult> Edit(int id, [Bind("Id,Name,IsComplete")] Todoitems todo)
        {
            if (ModelState.IsValid)
            {
